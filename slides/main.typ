...
#speaker-note[
  ...
  + Методы глубокого обучения потенциально позволят преодолеть эти ограничения без ухудшения производительности.
]
...
#speaker-note[
  ...
  + Для этого необходимо разработать ПО для генерации томографических данных и произвести датасет.

  + Далее, нужно разработать и обучить основную модель, а также реализовать несколько альтернативных.

  + После этого -- произвести сравнительную оценку полученных моделей.
]
...
#speaker-note[
  + Целью нейросетевой модели здесь является предсказание метки. Так как она двоичная, при обучении необходимо минимизировать бинарную кросс-энтропию.

  + После обучения для конфигураций вычисляются модельные и эталонные показатели качества -- статистики Уилкоксона, численно равные ROC AUC.

  + Имея оценки качества для тестовой выборки, можно оценить адекватность модели, то есть соответствие наблюдателю-человеку, с помощью линейного коэффициента корреляции.

  + Цель можно сформулировать как максимизацию адекватности на тестовой выборке с помощью модели, минимизирующей потери на тренировочной.
]
...
#speaker-note[
  ...
  + У такой архитектуры есть недостаток -- для обучения И использования модели требуется большое количество однотипных изображений, так как для каждой локации сигнала относительно фона нужна отдельная модель.
]
...
#speaker-note[
  + В качестве базовой архитектуры для основной модели была выбрана свёрточная нейронная сеть с добавлением соединения быстрого доступа для увеличения производительности.

  + Эта модель, в отличие от классических наблюдателей, позволяет оценить качество почти любого набора изображений.

  + Снизу также представлены её основные компоненты.
]
...
#speaker-note[
  + На данном слайде представлена блок-схема программной системы, которая выполняет оценку качества изображений, а также перечислены средства, использованные при разработке.

  + Особое внимание здесь стоит уделить XCIST -- Python-библиотеке для физической симуляции компьютерного томографа.
]
...
#speaker-note[
  + Слева на слайде представлены графики отклонения модельных оценок от оценок наблюдателя-человека, посередине - диаграмма средней производительности наблюдателей, а справа -- вычисленные коэффициенты корреляции.
  ...
  + Модель CHO же имеет критически низкую среднюю производительность на имеющихся данных, из-за чего её третье место ставится под сомнение.
]
...
#speaker-note[
  ...
  + Также был создан датасет из 10400 записей, пятая часть из которых была размечена вручную, хоть и не очень хорошо.
